...
class Dynspec:
    def __init__(self, dsfile=None, fsfile=None):
        if dsfile:
            self.dsfile = dsfile
            self.dynspec, self.dynspec_err, self.T, self.F, self.source = self.read_psrflux(dsfile)
        elif fsfile:
            self.fsfile = fsfile
            self.foldspec, self.F, self.T, self.source, self.telescope = self.read_psrarch(fsfile)
            self.fsclean, self.flag, self.mask, self.bg, self.bpass = self.clean_foldspec()
            self.dynspec = self.create_dynspec(self.fsclean)
        else:
            print("Need either a dynspec file, or a folded spectrum")
    ...
    def clean_foldspec(I, plots=True, apply_mask=False, rfimethod='var', flagval=10, offpulse='True', tolerance=0.5, off_gates=0):
        ...
        if rfimethod == 'var':
            ...
            # Mask all values over flag_std sigma of the mean
    ...
    def create_dynspec(foldspec, template=[1], profsig=5., bint=1, binf=1):
        """
        Create dynamic spectrum from cleaned folded data cube
    ...
    def write_psrflux(self, fname, psrname=None, telname=None, note=None):
        ...
        """
        T_minute = (self.T.unix - self.T[0].unix)/60.
        dt = (T_minute[1] - T_minute[0])/2.
        T_minute = T_minute + dt
        F_MHz = self.F.to(u.MHz).value
        ...
        with open(fname, 'w') as fn:
            ...
            for i in range(len(T_minute)):
                ...
                for j in range(len(self.F)):
                    ...
                    di = self.dynspec[i, j]
                    di_err = self.dynspec_err[i, j]
                    fn.write("{0} {1} {2} {3} {4} {5}\n".format(i, j, 
                                                ti, fi, di, di_err) )
        print("Written dynspec to {0}".format(fname))    


    def rfifilter_median(self, xs=20, ys=4, sigma=3., fsigma=5., tsigma=0., iters=3):
        """
        ...
        ds_med = median_filter(self.dynspec, size=[xs,ys])
        gfilter = self.dynspec-ds_med
    ...
    def plot_diagnostics(self):
        ...
        plt.subplot(231)
        plt.plot(self.foldspec.mean(0).mean(0), color='k')
        plt.xlabel('phase (bins)')
        plt.ylabel('I (arb.)')
        plt.title('Pulse Profile')
        plt.xlim(0, self.foldspec.shape[-1])
        
        plt.subplot(232)
        plt.title('RFI flagging parameter (log10)')
        plt.xlabel('time (bins)')
        plt.ylabel('freq (bins)')
        plt.imshow(np.log10(self.flag).T, aspect='auto')

        plt.subplot(233)
        plt.title('Manual off-gate scaling')
        plt.imshow(self.mask.T, aspect='auto', cmap='Greys')
        plt.xlabel('time (bins)')
        
        plt.subplot(234)
        plt.imshow(self.foldspec.mean(0), aspect='auto')
        plt.xlabel('phase')
        plt.ylabel('freq')

        plt.subplot(235)
        plt.imshow(self.foldspec.mean(1), aspect='auto')
        plt.xlabel('phase')
        plt.ylabel('time')

        plt.subplot(236)
        plt.imshow(self.foldspec.mean(2).T, aspect='auto')
        plt.xlabel('time')
        plt.ylabel('freq')
    
    def plot_secspec(self, xlim=None, ylim=None, bintau=2, binft=1, vm=3.,
